...
public class PasswordDecoderFilter extends AbstractGatewayFilterFactory {
    ...
    public GatewayFilter apply(Object config) {
        return (exchange,chain) ->{
            ...
            return bodyInserter.insert(bodyOutputMes,new BodyInserterContext()).then(Mono.defer(() ->{
                ServerHttpRequest decoder = decorate(exchange,headers,bodyOutputMes);
                return chain.filter(exchange.mutate().request(decoder).build());
            }));
        };
    }
    ...
    private Function decryptAES() {
        return s -> {
            ...
            Mono<String> just = Mono.just(HttpUtil.toParams(inParamsMap, Charset.defaultCharset(), true));
            log.info("decryptAES() method return str is ========>:{}",just);
            return just;
        };
    }
