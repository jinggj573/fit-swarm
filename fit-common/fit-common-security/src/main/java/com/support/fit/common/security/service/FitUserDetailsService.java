...
public interface FitUserDetailsService extends UserDetailsService, Ordered {
    /**
     * 是否支持此客户端校验
    ...
    default UserDetails getUserDetails(CommonResult<UserInfo> result) {
        ...
        // 构造security用户
        return new FitUser(user.getUserId(), user.getDeptId(), user.getUsername(),
     ...
     * @return
     */
    default UserDetails loadUserByUser(FitUser fitUser) {
        return this.loadUserByUsername(fitUser.getUsername());
    }
